...
def set_link_color(link,color,robot=0):
	...
	if len(color) >= 4:
		...
		mat["opacity"] = color[3]
	sceneIndex = -1
	#TODO: get the real path of the link in the scene!
	#need to traverse the scene graph, dammit...
	_set_path(('object',link.getName(),'material'),mat)
	print "Setting color!",('object',sceneIndex,'material'),mat
...
def _init(world):
	global _world,_sceneCurrent
	_world = world
	_sceneCurrent = json.loads(world.getSceneJSON())

def _getInitialJSON():
	global _world,_sceneCurrent
	return json.dumps(_sceneCurrent)

def _getUpdateJSON():
	global _world,_sceneCurrent,_sceneChanges
	msg = json.loads(_world.getTransformsJSON())
	msg['changes'] = _sceneChanges
	_sceneChanges = []

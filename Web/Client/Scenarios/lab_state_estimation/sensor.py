...
import math
import random
from common import CameraBlob,CameraColorDetectorOutput,ObjectPositionOutput,OmniscientObjectOutput
...
class ObjectPositionSensor:
    def __init__(self):
        self.noise = 0.1
    def emulate(self,sim):
        """Given a simulator instance, emulates the sensor.
        Result is a ObjectPositionOutput structure."""
        global objectColors
        names = []
        positions = []
        for i in range(sim.world.numRigidObjects()):
            ...
            names.append(name)
            noise = [random.uniform(-self.noise,self.noise) for k in range(3)]
            positions.append(vectorops.add(o.getTransform()[1],noise))
        return ObjectPositionOutput(names,positions)

...
class WebSharePath:
	def __init__(self,filename="path.html",boilerplate="share_path_boilerplate.html",name="Klamp't Three.js app"):
		self.name = name
	...
	def start(self,world):
		...
		if isinstance(world,Simulator):
			...
			self.sim = None
			self.world = world
		self.scene = self.world.getSceneJSON()
	def animate(self,time=None):
		...
		while dt >= self.dt:
			numadd += 1
			transforms = json.loads(self.world.getTransformsJSON())
...
if __name__ == '__main__':
	...
	if len(sys.argv) == 1:
		...
		sim = Simulator(world)
		share = WebSharePath(name="Klamp't test path")
		share.start(sim)
		for i in range(100):
			sim.simulate(0.02)
			share.animate()
		...
		assert len(sys.argv) == 3,"Usage: share_path world.xml robot_path"
		...
		share = WebSharePath(name="Klamp't path "+os.path.split(sys.argv[2])[1])
		share.start(world)
		share.dt = dt
		t = traj.times[0]

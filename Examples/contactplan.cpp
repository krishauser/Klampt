...
///A margin used in StancePlan on the distance between the COM and the edge of the support polygon
		 ...
		 const HaltingCondition& cond,const string& plannerSettings="")
{
  ...
  ContactCSpace transitionCspace(world,robot,&settings); 
  ...
  if(addedHolds.empty()) {
    //no added holds! no change needed
    path.edges.resize(1);

...
class IKSolver
{
 public:
  ...
  bool minimize();
  /** Tries to find a configuration that satifies all simultaneous objectives
   * up to the desired tolerance. Amongst configurations on the solution
   * manifold, this tries to minimize the secondary objective function.  The
   * gradient must also be given.
   * 
   * The user provides a pair of functions ``(f,grad)`` with ``f(q)`` the secondary
   * objective to minimize and ``grad(q)`` its gradient.  Here q is a function of
   * all robot DOFs, and ``grad(q)`` should return a list or tuple of length `len(q)``.

...
def setParabolic():
    traj = trajectory.path_to_trajectory(traj0,velocities='parabolic',speed=1.0)
    vis.animate("robot",traj)
    vis.addText("label","Parabolic velocity profile")
def setMinJerk():
    traj = trajectory.path_to_trajectory(traj0,velocities='minimum-jerk',speed=1.0)
    vis.animate("robot",traj)
    vis.addText("label","Minimum-jerk velocity profile")
def setStartStop():
    traj = trajectory.path_to_trajectory(traj0,speed=1.0,zerotol=0.0)
    vis.animate("robot",traj)
    vis.addText("label","Start/stop trajectory")
def setHermite():
    ...
    #dtraj = trajectory.Trajectory(dtraj.times,dtraj.milestones)
    traj = trajectory.path_to_trajectory(dtraj,speed=1.0,zerotol=20.0)
...
class MyVisPlugin(GLPluginInterface):
    def __init__(self):
        ...
        self.add_action(setParabolic,"Parabolic",'p')
        self.add_action(setMinJerk,"Minimum jerk",'m')
        self.add_action(setStartStop,"Start/stop",'s')
        self.add_action(setHermite,"Hermite spline",'h')

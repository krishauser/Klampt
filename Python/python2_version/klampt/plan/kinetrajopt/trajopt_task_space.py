...
class PoseConstraint(ConstrInterface):
    ...
    def __init__(self, wrobot, linkid, target_pose):
...
class DirectionConstraint(ConstrInterface):
    ...
    def __init__(self, wrobot, linkid, lcl_dir=[0, 0, 1], world_dir=[0, 0, 1]):
        assert isinstance(wrobot, MaskedRobot)
        self.linkid = linkid
        self.robot = wrobot
...
class OrientationConstraint(ConstrInterface):
    ...
    def __init__(self, robot, linkid, R):
        self.robot = robot
        self.linkid = linkid
        self.link = self.robot.link(linkid)
        self.target_R = R
...
class PositionConstraint(ConstrInterface):
    ...
    def __init__(self, wrobot, linkid, lcl_pos, world_pos):
        assert isinstance(wrobot, MaskedRobot)
        self.robot = wrobot
        self.linkid = linkid

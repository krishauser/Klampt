...
bool OrXmlKinbody::GetObjectOrTerrain(RobotWorld& world) {
	if (!isGetAllCleanBody) {
		LOG4CXX_INFO(KrisLibrary::logger(),"Error: should first process all the contents by GetAllCleanBodyJoints!\n" 
			<<"\n");
		if(KrisLibrary::logger()->isEnabledFor(log4cxx::Level::ERROR_INT)) getchar();
		return false;
	}
...
//		LOG4CXX_INFO(KrisLibrary::logger(),"body "<<name<<" size "<<xmlGeoms.size());if(KrisLibrary::logger()->isEnabledFor(log4cxx::Level::ERROR_INT)) getchar();
...
bool OrXmlRobot::applyTran2Tparent() {
	if (!transformation)
		return true;
	if (!allclean) {
		LOG4CXX_INFO(KrisLibrary::logger(),	"Error: robot transformation should be applied after everything else is processed!\n");
		if(KrisLibrary::logger()->isEnabledFor(log4cxx::Level::ERROR_INT)) getchar();
		return false;
	}
	transformation->ComputeTransform();
...
//			if(xmlBodys[i]->Tparent)LOG4CXX_INFO(KrisLibrary::logger(),*xmlBodys[i]->Tparent);if(KrisLibrary::logger()->isEnabledFor(log4cxx::Level::ERROR_INT)) getchar();
//		}
//	}
//	file.close();

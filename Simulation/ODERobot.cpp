...
void ODERobot::Create(int robotIndex,dWorldID worldID,bool useBoundaryLayer)
{
  ...
  for(size_t i=0;i<bodyLinks.size();i++) {
    ...
    if(geometry[primaryLink] != NULL) {
      ...
      //LOG4CXX_INFO(KrisLibrary::logger(),"Robot "<<robotIndex<<" link "<<primaryLink<<" GeomData set to "<<RobotIndexToGeomData(robotIndex      //set defaults
	  ...
	  if(robot.joints[i].type != RobotJoint::Spin) {
		...
		LOG4CXX_WARN(KrisLibrary::logger(),"ODERobot: Warning, turning off joint stops on joint "<<i<<" (link "<<robot.LinkName(link).c_str());
          ...
          if(robot.qMin(link) <= -Pi+0.1 || robot.qMax(link) >= Pi-0.1) {
            LOG4CXX_WARN(KrisLibrary::logger(),"ODERobot: Warning, robot joint limits on joint "<<i<<" (link "<<robot.LinkName(link) <<" are close to [-pi,pi].\n");       
...
void ODERobot::SetVelocities(const Config& dq)
{
  ...
  if(!temp.isEqual(dq,1e-4)) {
    ...
    LOG4CXX_INFO(KrisLibrary::logger(),"from GetVelocities = "<<temp<<"\n");
    LOG4CXX_INFO(KrisLibrary::logger(),"did you remember to set the robot's configuration?"<<"\n");
  }
...
void ODERobot::AddTorques(const Config& t)
{
  ...
  for(int i=0;i<t.n;i++)
    if(!IsFinite(t(i))) {
      LOG4CXX_ERROR(KrisLibrary::logger(),"Error, commanding link "<<i);
      if(KrisLibrary::logger()->isEnabledFor(log4cxx::Level::ERROR_INT)) getchar();
      return;
    }
